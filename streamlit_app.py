...
if df is not None and len(df) > 0:
    ...
    if model_results:
        ...
        forecast_test_df = model_results['forecast_test_df']
        forecast_future = model_results['forecast_future']
        ...
        with col1:
            st.metric("💰 Current Price", f"${train_df['y'].iloc[-1]:.2f}")
        with col2:
            st.metric("📊 10-Year Average", f"${train_df['y'].mean():.2f}")
        with col3:
            st.metric("🎯 10-Year High", f"${train_df['y'].max():.2f}")
        ...
        with metric_col3:
            st.metric("Training Period", "~9.25 Years")
        with metric_col4:
        ...
        ax1 = axes[0]
        if show_historical:
            ax1.plot(train_df['ds'], train_df['y'], label='Training Data (9.25 years)', 
                    color='blue', linewidth=1.5)
        if show_test_predictions:
            ax1.plot(test_df['ds'], test_df['y'], label='Test Data (6 months - Actual)', 
                    color='green', linewidth=1.5)
            ax1.plot(forecast_test_df['ds'], forecast_test_df['yhat'], 
        ...
        ax1.set_title('Silver Price: Training & Test Data with Predictions', 
        ...
        ax2 = axes[1]
        if show_historical:
            ax2.plot(train_df['ds'], train_df['y'], label='Historical Data', 
                    color='blue', linewidth=1)
        if show_test_predictions:
            ax2.plot(test_df['ds'], test_df['y'], color='green', linewidth=1.5)
            ax2.plot(forecast_test_df['ds'], forecast_test_df['yhat'], 
                    label='Test Predictions', color='red', linewidth=1.5, linestyle='--')
        if show_forecast:
            ...
            ax2.fill_between(forecast_future['ds'], forecast_future['yhat_lower'], 
                            forecast_future['yhat_upper'], alpha=0.2, color='orange', 
                            label='95% Confidence Interval')
        
        ax2.axvline(x=test_df['ds'].iloc[0], color='gray', linestyle=':', alpha=0.7, 
                   label='Test Set Start')
        ax2.axvline(x=df.iloc[-1]['ds'], color='purple', linestyle=':', alpha=0.7, 
        ...
        if st.checkbox("Show Raw Data Tables"):
            ...
            st.write("**Last 10 Days of Forecast**")
            st.dataframe(forecast_future.tail(10).reset_index(drop=True), use_container_width=True)
            
            st.write("**Last 10 Days of Test Predictions**")
            test_results = pd.DataFrame({
                'Date': forecast_test_df['ds'].values,
                'Actual': test_df['y'].values,
                'Predicted': forecast_test_df['yhat'].values,
                'Error': test_df['y'].values - forecast_test_df['yhat'].values
            })
            st.dataframe(test_results.tail(10).reset_index(drop=True), use_container_width=True)
    ...
    st.error("Failed to load silver price data. Please refresh the page.")
